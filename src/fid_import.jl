""" fid_import_correct(s <: BrukerSpectrum, zf = TD) -> s, ϕ₁
-----------------------------------------------------------------------------------
...
Must be called _after_ the FID is made complex. By default, `applies zf equivalent 
to the SI value, here.`
Also returns 
- n.b. Calling zero_fill() on a BrukerSpectrum is a thin wrapper to this function.
...
"""
function fid_import_offset_correct(s :: BrukerSpectrum, zf :: Int = s["SI"])
    zf = zf > s["TD"] ? zf : s["TD"] && @warn("Cannot zero fill less than TD. 
                                              \nSetting zf = TD.")
    ...
    td = length(s.fid)
    pre_acq = s.fid[1:points]
    fid[1:td-points] .= s.fid[points+1:end]
    # apply first-order phase offset.
    #fid .*= exp.(-1im * ϕ₁ .* eachindex(fid))
    fid[end-points:end] .= pre_acq
    s.fid = fid
    return s, ϕ_dsp
end

"""getoffset(s <: BrukerSpectrum) -> f64
------------------------------------------------------------------------------------
Get the GRPDLY offset or look it up.
"""
function getoffset(s :: BrukerSpectrum)
    # simplest case - return GRPDLY iff ∃d > 0.
    haskey(s, "GRPDLY") && s["GRPDLY"] < 0 ? nothing : return s["GRPDLY"] 
    # otherwise
    row = findfirst(isequal(s["DECIM"]), DSP_TABLE)
    col = s["DSPFVS"] - 9 # Only the 10, 11, 12 cases are listed, why be fancy?
    try 
        return DSP_TABLE[row,col]
    catch err
...
export fid_import_offset_correct
